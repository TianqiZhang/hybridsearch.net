...
queryCommand.SetHandler(async (InvocationContext context) =>
{
    ...
    if (embeddingProvider is null)
    {
        ...
        Console.Error.WriteLine("  Set HYBRIDSEARCH_AZURE_OPENAI_ENDPOINT, HYBRIDSEARCH_AZURE_OPENAI_KEY, and");
        Console.Error.WriteLine("  HYBRIDSEARCH_AZURE_OPENAI_DEPLOYMENT environment variables, or use");
...
static IEmbeddingProvider? CreateEmbeddingProvider(string providerName)
{
    ...
    {
        var endpoint = Environment.GetEnvironmentVariable("HYBRIDSEARCH_AZURE_OPENAI_ENDPOINT");
        var key = Environment.GetEnvironmentVariable("HYBRIDSEARCH_AZURE_OPENAI_KEY");
        var deployment = Environment.GetEnvironmentVariable("HYBRIDSEARCH_AZURE_OPENAI_DEPLOYMENT");
        ...
        if (string.IsNullOrEmpty(endpoint) || string.IsNullOrEmpty(key) || string.IsNullOrEmpty(deployment))
        {
            ...
            Console.Error.WriteLine("  HYBRIDSEARCH_AZURE_OPENAI_ENDPOINT");
            Console.Error.WriteLine("  HYBRIDSEARCH_AZURE_OPENAI_KEY");
            Console.Error.WriteLine("  HYBRIDSEARCH_AZURE_OPENAI_DEPLOYMENT");
            return null;
        }
